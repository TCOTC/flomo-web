...
let pluginRef: FlomoPlugin | null = null;
...
export function registerFlomoDock(plugin: FlomoPlugin) {
    ...
    <div class="b3-typography flomo-web__fallback">${i18n.desktopOnly}</div>
            ...
            const startSrc = pendingUrl || FLOMO_URL;
            pendingUrl = "";
            root.innerHTML = `<div class="fn__flex-1 fn__flex-column flomo-web">
    <div class="block__icons">
        <div class="block__logo">
    ...
    <div class="fn__flex-1 flomo-web__stage">
        <webview class="flomo-web__webview" src="${startSrc}" partition="${WEBVIEW_PARTITION}"></webview>
...
function showFlomoDock() {
    if (!pluginRef) {
        return;
    }
    const type = pluginRef.name + DOCK_TYPE;
    const icon = document.querySelector(`.dock__item[data-type="${type}"]`) as HTMLElement | null;
    if (icon && !icon.classList.contains("dock__item--active")) {
        icon.click();
    }
}
...
/** 在停靠栏打开 flomo 链接；停靠栏未展开时会先展开 */
export function openFlomoUrl(url: string): boolean {
    const abs = parseFlomoUrl(url);
    if (!abs || !isElectronDesktop()) {
        return false;
    }
    pendingUrl = abs;
    showFlomoDock();
...
export function bindFlomoLinkClicks(): () => void {
